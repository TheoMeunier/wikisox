<x-guest-layout>
    <section class="flex items-center justify-around">
                ...
                <div class="block mt-4">
                    <label for="remember_me" class="inline-flex items-center">
                        <input id="remember_me" type="checkbox"
                               ...
                               name="remember">
                        <span class="ml-2 text-sm text-gray-600">{{ __('input.label.remember_me') }}</span>
                    </label>
                </div>
                    ...
                    @if (Route::has('password.request'))
                        <a class="underline text-sm text-gray-600 hover:text-gray-900"
                           href="{{ route('password.request') }}">
                            {{ __('button.action.forgotPassword') }}
                        </a>
                    @endif

                    <x-button class="ml-3">
                        {{ __('button.action.login') }}
                    </x-button>
                </div>
            </form>
        </x-auth-card>
        ...
        <div class="w-2/4">
            <img src="{{ asset('images/logo_idea.svg') }}" alt="image view login">
        </div>
    </section>
</x-guest-layout>
